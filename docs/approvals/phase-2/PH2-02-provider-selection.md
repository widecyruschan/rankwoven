...
| 即時內容評分、結構化輸出、工具編排          | OpenAI Responses，預設 `gpt-5.6-luna`；高質量使用 `gpt-5.6-terra` | Claude Sonnet 5                  | input/output token           | OpenAI 主 Provider；所有 schema、refusal、截斷及 `store:false` 由 adapter 處理 |
| Embedding、聚類、相似度                     | OpenAI Embeddings                                                 | Gemini Embedding 2 作評估候選    | token                        | 先用 OpenAI embedding；以內容 hash + model version 快取                        |
| 低成本非同步批量                            | Gemini 3.8／3.7 Flash Batch；OpenAI Batch 作同能力備選            | Anthropic Message Batches        | token、batch job             | 僅用於可延遲任務，不用於即時 UI 回應                                           |
| 長文引用對齊、高價值改寫                    | Anthropic Claude Sonnet 5                                         | OpenAI `gpt-5.6-terra`           | input/output token           | 只作明確路由的高質量 fallback，不靜默替換來源                                  |
| Google 搜尋 grounding                       | Gemini Google Search grounding                                    | OpenAI Web Search                | search request + model token | P0 不預設開啟；只有需要即時引用的任務才按次計費                                |
...
| Provider／模型             |                                                     Standard input | Standard output | Batch／備註                                                                         | 資料與配額邊界                                                         |
| -------------------------- | -----------------------------------------------------------------: | --------------: | ----------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| OpenAI `gpt-5.6-luna`      |                                                        $0.20／MTok |     $1.20／MTok | Batch $0.10／$0.60；Batch 最多 50,000 requests、200 MB、24 小時窗口                 | 互動主模型；敏感工作流使用 `store:false`；資料區域 endpoint 可能加 10% |
| OpenAI `gpt-5.6-terra`     |                                                        $2.00／MTok |    $12.00／MTok | Batch $1.00／$6.00                                                                  | 高質量 fallback；不要把 fast mode 當成本優化，因為費用更高             |
| OpenAI Web Search          |                                                                  - |               - | $10／1,000 calls，搜尋內容 token 另按模型計費                                       | 只有明確需要即時 web evidence 才使用                                   |
| Anthropic Claude Sonnet 5  |                                                           $2／MTok |       $10／MTok | Batch $1／$5；單批最多 100,000 requests 或 256 MB，最長 24 小時                     | 結果可下載 29 日；按 workspace rate limit 與 spend limit 治理          |
| Anthropic Claude Haiku 4.5 |                                                           $1／MTok |        $5／MTok | Batch $0.50／$2.50                                                                  | 可作低成本分類 fallback，但不取代 OpenAI 主路由                        |
| Gemini 3.8／3.7 Flash      | 官方頁列示截至 2026-12-31 $0.75／$3.75；2027-01-01 起 $1.50／$7.50 |            同左 | Batch 為 standard 約 50%；Google Search 每月共 5,000 requests 免費，之後 $14／1,000 | 以官方 model ID 與生效日期保存 snapshot；不要在程式碼寫死 alias        |
...
| ---------------------------------- | ----------------------------------------------- | -------------------------------------------------------- | ------------------------------------------------------ |
| 即時 SEO 分析、JSON schema、短改寫 | OpenAI `gpt-5.6-luna`                           | OpenAI `gpt-5.6-terra` 或 Claude Haiku 4.5（需政策相容） | 將自由文本截取成 JSON；拒答當成功                      |
| 高價值長文、引用對齊               | Claude Sonnet 5                                 | OpenAI `gpt-5.6-terra`                                   | 沒有來源時自動補數字、案例或作者資格                   |
| 大量意圖分類、摘要、標籤           | Gemini Flash Batch                              | OpenAI Batch `gpt-5.6-luna`                              | 用 batch 處理需要即時回應的互動操作                    |
| 多語言 embedding／聚類             | OpenAI Embedding + deterministic clustering     | Gemini Embedding 2 fixture 評估                          | 每次由模型自由命名 cluster，導致結果不可重現           |
| 競品排名、搜尋量、backlink         | DataForSEO                                      | Ahrefs／Semrush BYOK                                     | 用 AI 估算缺失的真實指標；混合不同 Provider 的時間序列 |
| 即時引用／GEO 監控                 | DataForSEO LLM Mentions 或明確 Gemini grounding | OpenAI Web Search                                        | 把 grounding 結果當成 Google 排名保證                  |

Fallback 必須在同一 capability、資料權限、保存條款及地區政策內；切換後保存實際 Provider／model。若無相容 fallback，返回 `PROVIDER_UNAVAILABLE` 或 `partial`，不得靜默猜值。
...
```text
ai_cost = input_tokens / 1_000_000 * input_price
        + output_tokens / 1_000_000 * output_price
        + search_requests * search_unit_price
        + cache_storage_cost
...
reserved_credits >= estimated_provider_cost * safety_multiplier
```

預估時使用 1.25 safety multiplier；實際結算使用供應商回傳的 usage／units。失敗未產生成本的 reservation release；已產生部分成本則 finalize 實際值並標記 `partial`。

### 5.2 1,000 次內容分析

假設每次 4,000 input tokens + 1,000 output tokens，合計 4M input + 1M output；未計圖片、grounding、重試、cache、稅項及基礎設施：

| 路由                               | 同步估算 | Batch 估算 | 用途                    |
| ---------------------------------- | -------: | ---------: | ----------------------- |
| OpenAI `gpt-5.6-luna`              |    $2.00 |      $1.00 | 預設互動／批量基線      |
| OpenAI `gpt-5.6-terra`             |   $20.00 |     $10.00 | 高質量內容與複雜 schema |
| Claude Sonnet 5                    |   $18.00 |      $9.00 | 引用對齊及長文改寫      |
| Claude Haiku 4.5                   |    $9.00 |      $4.50 | 低風險分類／摘要        |
| Gemini 3.8／3.7 Flash（2026 價格） |    $6.75 |   約 $3.38 | 大批量標籤、摘要、初稿  |
...
- Provider 429、timeout、schema refusal、價格漂移及地區不可用都要進入可觀測事件；重試最多一次格式修復及一次具退避的 provider retry。
- 超過全平台 cap 時停用高成本任務，保留讀取既有結果、匯出及回滾能力。
- DataForSEO、Ahrefs、Semrush、AI model 指標分開時間序列；UI 顯示原生來源和估算標籤。
...
- AI 任務保存最小必要輸入；OpenAI 敏感流程使用 `store:false`，RankWoven 保存版本化結果及審計參照。
...
2. **AI**：OpenAI `gpt-5.6-luna` 作互動與 embedding 主路由；Gemini Flash Batch 作非同步低成本批量；Claude Sonnet 5 作引用和高質量 fallback。
...
- 以 provider fixtures 鎖定 success、timeout、429、partial、schema refusal、價格漂移及 unavailable region。
- 於 PH2-03 把 capability adapter、pricing snapshot、usage ledger 及 quota API 寫成契約；本文件不授權 runtime 實作。

## 9. 核檢結果

- [x] 不同 Provider 的指標不混入同一時間序列。
- [x] 價格已標記為公開固定、按量、最低充值或需登入／報價。
- [x] 每項外部任務有 fallback 或明確 unavailable 行為。
- [x] Provider 更新不會改變歷史請求的真實來源標籤。
- [x] 已提供月度、workspace、每日 cap、告警與 hard stop。
- [x] BYOK、secret store、資料最小化及人工批准邊界已定義。
- [x] 官方頁面於 2026-09-13 重新核對；動態價格需在實作前再次刷新。

**Open risks**：正式 DataForSEO 商務條款、Ahrefs／Semrush 報價、實際套餐 credits 與 30 日毛利尚未取得；這些不阻塞 PH2-03 的契約設計，但阻塞正式放量及平台代付上限提升。
