...
10. 補齊可重複部署流程，至少記錄部署版本、部署時間、回滾目錄和健康檢查結果，避免生產環境再次停留在舊構建。
11. 處理 Docker build 過程中 `npm audit` 提示的高危依賴，先評估是否可在不破壞現有功能的情況下升級。
12. 將客戶後台同步頁後續接入真正的手動同步任務 API，而不是只刷新同步狀態。
