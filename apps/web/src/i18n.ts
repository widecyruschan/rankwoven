...
const messages = {
  en: {
    ...
    siteAudit: {
      ...
      ahrefsConfigHint: 'Enable this to make the primary audit use the connected Ahrefs project instead of the limited fallback crawler.',
      ahrefsProjectRequired: 'Enter the Ahrefs project ID before enabling full-site audit.',
      ahrefsUnavailable: 'Ahrefs full-site audit is not available. Check the API key and project configuration.',
      ahrefsConfirmationContent: 'Use the configured Ahrefs Site Audit project to refresh this full-site report. Affected URLs load only when an issue is expanded, which controls API usage. Continue?',
  ...
  'zh-Hant': {
    ...
    siteAudit: {
      ...
      ahrefsDatePlaceholder: '2026-09-12T02:21:42Z',
      ahrefsConfigHint: '啟用後，「立即執行檢測」會使用 Ahrefs 全站專案，而非受限的備援爬蟲。',
      ahrefsProjectRequired: '啟用全站檢測前，請先輸入 Ahrefs 專案 ID。',
      ahrefsUnavailable: 'Ahrefs 全站檢測暫不可用，請檢查 API 金鑰與專案配置。',
      ahrefsConfirmationContent: '將使用已配置的 Ahrefs Site Audit 專案更新全站報告；僅在展開問題時載入受影響 URL，以控制 API 用量。確認繼續？',
